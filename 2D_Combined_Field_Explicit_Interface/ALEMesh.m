function E=ALEMesh(dof,Elem)
% return value contains E.nVertex, N.freeN, E.fixedS
% elastic stiffness matrix for the fluid. Use P1.
deg = 1;
Elem.p1p2 = dof(Elem.elem2dof(:,2),:)-Elem.dof(Elem.elem2dof(:,1),:); 
Elem.p3p1 = dof(Elem.elem2dof(:,1),:)-Elem.dof(Elem.elem2dof(:,3),:); 
Elem.detT = Elem.p1p2(:,2).*Elem.p3p1(:,1)-Elem.p1p2(:,1).*Elem.p3p1(:,2);

% Since it is P1, there is no iso element
NNT=(deg+1)*(deg+2)/2; % is the number of nodes on each triangle
strBaseName=['base',num2str(deg),'.mat'];
load(strBaseName);
N = size(unique(Elem.elem2dof(:,1:3)),1);
