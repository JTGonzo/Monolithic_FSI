function [rr]=LiftDrag(Elem,Phy,mix,u,p,DeEF,level)
% lift and drag coefficients for flow past a cylinder with flag

NNT=Elem.nnt; % is the number of nodes on each triangle
elem = Elem.itfElem;

x=zeros(size(elem,1),NNT);
for i=2:3
    x(:,i)=Elem.dof(Elem.elem2dof(elem,i),1);
end

e = find(x>=level);
e(e>length(Elem.itfElem) & e<=2*length(Elem.itfElem))=e(e>length(Elem.itfElem) & e<=2*length(Elem.itfElem))-length(Elem.itfElem);
e(e>2*length(Elem.itfElem))=e(e>2*length(Elem.itfElem))-2*length(Elem.itfElem);
elem = Elem.itfElem( unique(e) );
NT=length(elem);   
...
for i=1:bdQuadN
    ...
    nn=[-tau(:,2),tau(:,1)]./[jacobian,jacobian]; % normal vector
    tt=[nn(:,2),nn(:,1)];
    % p=[qtx*psi',qty*psi'];
...
%     rr1=rr1+sum(repmat(delta*jacobian,1,2).*RR1,1);
