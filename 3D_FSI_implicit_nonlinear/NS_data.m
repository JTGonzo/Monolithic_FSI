...
% Dirichlet
data.bcDir_t  = @(t)( 0.5*(1-cos(2*pi*t)).*(t<=0.5) + 1 * (t>0.5));

data.bcDir{1} = @(x, y, z, t, param)(data.bcDir_t(t) * U_bar*y.*(H-y)/(H/2)^2.*(x==0) + 0.*x.*y); 
...
data.Output.DragLift.factor          = 2/(10^3*1^2*0.1);
